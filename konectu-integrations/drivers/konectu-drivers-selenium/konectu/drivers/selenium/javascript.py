...
if (!window._konectu_move_listener) {
    window._konectu_move_listener = function() {
        const bbs = document.querySelectorAll('.konectu-highlight');
        bbs.forEach(bb => {
    ...
    window.addEventListener('scroll', window._konectu_move_listener);
    window.addEventListener('resize', window._konectu_move_listener);
}
"""

REMOVE_HIGHLIGHT = """
if (window._konectu_move_listener) {
    window.removeEventListener('scroll', window._konectu_move_listener);
    window.removeEventListener('resize', window._konectu_move_listener);
    delete window._konectu_move_listener;
}
arguments[0].filter(a => a).forEach(a => a.style.cssText = a.dataset.originalStyle || '');
document.querySelectorAll('.konectu-highlight').forEach(a => a.remove());
"""
...
def get_highlighter_style(color: str = "red", label: bool = False):
    ...
    bb.className = 'konectu-highlight';
