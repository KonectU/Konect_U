...
from konectu.core.utilities.web_utils import display_screenshot
...
from konectu.core.extractors import DynamicExtractor
    ...
    def perform_fallback(self, prompt, instruction) -> str:
        ...
        for i in range(self.n_search_attempts):
            ...
            if screenshot_folder == self.driver.get_current_screenshot_folder():
                raise ValueError(
                    "Temporary Python Engine screenshot folder must not be the same as the konectu default screenshot folder"
                )
