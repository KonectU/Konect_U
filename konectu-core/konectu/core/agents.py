...
from konectu.core.utilities.format_utils import (
    extract_before_next_engine,
    extract_next_engine,
    extract_world_model_instruction,
    replace_hyphens,
)
from konectu.core.logger import AgentLogger, LocalDBLogger
from konectu.core.memory import ShortTermMemory
from konectu.core.base_driver import BaseDriver
from konectu.core.base_engine import ActionResult
from konectu.core.utilities.telemetry import send_telemetry
from PIL import Image
from IPython.display import display, HTML, Code
from konectu.core.token_counter import TokenCounter
from konectu.core.utilities.config import is_flag_true

from konectu.core.utilities.profiling import (
    ChartGenerator,
    time_profiler,
    start_new_step,
...
class WebAgent:
    ...
    def demo(
        ...
        try:
            from konectu.gradio import GradioAgentDemo

            grad = GradioAgentDemo(objective, None, self, user_data, screenshot_ratio)
            grad.launch()
        except ImportError:
            raise ImportError(
                "`konectu-gradio` package not found, "
                "please run `pip install konectu-gradio`"
            )
    ...
    def run(
        ...
        user_data=None,
        display: bool = False,
        log_to_db: bool = is_flag_true("konectu_LOG_TO_DB"),
        step_by_step=False,
    ) -> ActionResult:
        self.interrupted = False
        ...
        try:
            ...
            origin = self.origin if hasattr(self, "origin") else "konectu"
    ...
    def get_summary(self):
        from konectu.core.utilities.profiling import agent_events, agent_steps
